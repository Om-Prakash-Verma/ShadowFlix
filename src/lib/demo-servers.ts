import type { DemoServer, PlayerSource } from "@/types/player";

export const demoServerList: DemoServer[] = [
  {
    id: 1,
    name: "Best Server",
...
export function getDemoPlayerSources({
  tmdbId,
  imdbId,
  mediaType,
  season = 1,
  ...
  return demoServerList.reduce<PlayerSource[]>((sources, server) => {
    const preferredId = server.useImdb ? imdbId : tmdbId;
    if (!preferredId) {
      return sources;
    }
    ...
    sources.push({
      id: `demo-${server.id}`,
      name: server.name,
      kind: server.kind ?? "embed",
      src:
        mediaType === "movie"
          ? server.movieLink(preferredId)
          : server.episodeLink(preferredId, season, episode),
      label: server.notes,
    });
